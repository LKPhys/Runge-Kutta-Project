...
def rkf45_bifurcation(f, t0, t1, x0, v0, A, omega, period, filename1, filename2, mu=0):
    """
    Adaptive Runge-Kutta Fehlberg algorithm.
    """
    ...
    for i in range(len(omega)):
        t = t0[i]
        period_array = [t + period[i]*melon for melon in range(100)]

        while t <= t1:
            new_o = 0
            new_o = omega[i]
            # Compute each k and l value, the order of this matters.
            k[0] = h * f(t, x, v, new_o, mu)[0]
            l[0] = h * f(t, x, v, new_o, mu)[1]

            k[1] = h * f(t + c0*h, x + c0*k[0], v + c0*l[0], new_o, mu)[0]
            l[1] = h * f(t + c0*h, x + c0*k[0], v + c0*l[0], new_o, mu)[1]

            k[2] = h * f(t + c1*h, x + c2*k[0] + c3*k[1], v + c2*l[0] + c3*l[1], new_o, mu)[0]
            l[2] = h * f(t + c1*h, x + c2*k[0] + c3*k[1], v + c2*l[0] + c3*l[1], new_o, mu)[1]

            k[3] = h * f(t + c4*h, x + c5*k[0] - c6*k[1] + c7*k[2], v + c5*l[0] - c6*l[1] + c7*l[2], new_o, mu)[0]
            l[3] = h * f(t + c4*h, x + c5*k[0] - c6*k[1] + c7*k[2], v + c5*l[0] - c6*l[1] + c7*l[2], new_o, mu)[1]

            k[4] = h * f(t + h, x + c8*k[0] - 8.0*k[1] + c9*k[2] - c10*k[3], v + c8*l[0] - 8.0*l[1] + c9*l[2] - c10*l[3], new_o, mu)[0]
            l[4] = h * f(t + h, x + c8*k[0] - 8.0*k[1] + c9*k[2] - c10*k[3], v + c8*l[0] - 8.0*l[1] + c9*l[2] - c10*l[3], new_o, mu)[1]

            k[5] = h * f(t + c11*h, x - c12*k[0] + 2.0*k[1] - c13*k[2] + c14*k[3] - c15*k[4], v - c12*l[0] + 2.0*l[1] - c13*l[2] + c14*l[3] - c15*l[4], new_o, mu)[0]
            l[5] = h * f(t + c11*h, x - c12*k[0] + 2.0*k[1] - c13*k[2] + c14*k[3] - c15*k[4], v - c12*l[0] + 2.0*l[1] - c13*l[2] + c14*l[3] - c15*l[4], new_o, mu)[1]
            ...
            t += h
            print('   ', t, end='\r')
            for pp in period_array:
                if t >= pp - pp*1E-1 and t <= pp + pp*1E-1:
                    fi.write('%f\n %f\n' % (x, omega[i]))
                    fi2.write('%f\n %f\n' % (v, omega[i]))
            if error <= e0:
                h = lam * h * (e0 / error)**0.2
